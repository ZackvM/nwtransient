...
function checkPostingUser($usrname, $passwrd) { 
    session_start();
    $uid = cryptservice ( $usrname, 'd' ); 
    $up =  cryptservice ( $passwrd, 'd' , true, $uid );
...
function registerServerIdent() {
   ...
   $sid = cryptservice($accid  ,'e', false );
   $sidp = cryptservice( $sesscode, 'e', true, $accid);  
   $rtn['u'] = $sid;
   $rtn['i'] = $sidp;   
   return $rtn;
...
function cryptservice( $string, $action = 'e', $usedbkey = false, $passedsid = "") {
    ...
    if ($usedbkey) {
        ...
        require(serverkeys . "/sspdo.zck");
        $sql = "select accesscode from serverControls.ss_srvIdents where idss_srvIdents = :sid";
        $rs = $conn->prepare($sql); 
